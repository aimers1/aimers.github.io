...
class QLWAPP_Backend {
	...
	function add_js() {
		if ( isset( $_GET['page'] ) && strpos( $_GET['page'], QLWAPP_DOMAIN ) !== false ) {
			...
			$admin = include_once QLWAPP_PLUGIN_DIR . 'assets/backend/js/admin.asset.php';

			wp_enqueue_style( 'qlwapp-admin', plugins_url( '/assets/backend/css/main.css', QLWAPP_PLUGIN_FILE ), array( 'wp-color-picker', 'qlwapp-select2' ), QLWAPP_PLUGIN_VERSION, 'all' );
			...
			wp_enqueue_script( 'qlwapp-admin', plugins_url( '/assets/backend/js/admin.js', QLWAPP_PLUGIN_FILE ), $admin['dependencies'], $admin['version'], true );
